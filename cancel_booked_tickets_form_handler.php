...
<body>
	...
	if (isset($_POST['Cancel_Ticket'])) {
		...
		if (empty($data_missing)) {
			...
			$query = "UPDATE ticket_details SET booking_status='CANCELLED' WHERE pnr=? and user_id=?";
			$stmt = mysqli_prepare($dbc, $query);
			...
			mysqli_stmt_close($stmt);
			if ($affected_rows == 1) {
				$query = "SELECT t.train_no,t.journey_date,t.no_of_passengers,t.class,0.85*p.payment_amount as refund_amount from ticket_details t,payment_details p WHERE t.pnr=? and t.pnr=p.pnr";
				$stmt = mysqli_prepare($dbc, $query);
				mysqli_stmt_bind_param($stmt, "s", $pnr);
				mysqli_stmt_execute($stmt);
				mysqli_stmt_bind_result($stmt, $train_no, $journey_date, $no_of_pass, $class, $refund_amount);
				mysqli_stmt_fetch($stmt);
				mysqli_stmt_close($stmt);
